...
Function Get-CompleteFileList {
	...
	$goodFiles = @()
	Foreach ($files in Get-ChildItem @switches -ErrorAction SilentlyContinue -ErrorVariable +badFile) {
...
Function Get-DirectorySize {
	...
	Foreach ($folder in $Folders) {
		...
		ForEach ($size in $sizes) {
			...
			If ($ShortNames) {
				If ($(Get-Item -LiteralPath $size).PSIsContainer -eq $false) {
					$SFSO = New-Object -ComObject Scripting.FileSystemObject
					$item = Get-Item -LiteralPath $($SFSO.GetFile($($size)).ShortPath)
				} Else {
					Write-Verbose "Not adding directory $size"
				}
			} Else {
				If ($(Get-Item -LiteralPath $size).PSIsContainer -eq $false) {
					$item = Get-Item -LiteralPath $($size)
				} Else {
					Write-Verbose "Not adding directory $size"
				}
			}
