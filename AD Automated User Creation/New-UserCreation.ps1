...
$XMLOptions = "32BCT.Options.xml" #change to desired XML file name
...
function Call-ANUC_pff {
	...
	$CSVTemplate.FileName = Join-Path $ParentFolder "32BCTusers.csv"
	$CSVTemplate.Filter = "CSV Files|*.csv|All Files|*.*"
	$CSVTemplate.ShowHelp = $True
	$CSVTemplate.Title = "Create CSV Template For 32BCT"
	$CSVTemplate.add_FileOk($CSVTemplate_FileOk)
	#
	# OFDImportCSV
	#
	$OFDImportCSV.FileName = Join-Path $ParentFolder "32BCTusers.csv"
